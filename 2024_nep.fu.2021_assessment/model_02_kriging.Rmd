...
* europa.txt
* pol.Labadie.IBP.csv
* nep2021_2013_2024_densities.csv 
* ggins_compiled_Labadie_contourplot.csv
* Labadie_Summary_ADG_2023_method.csv



Files after (inside "model_02"):
* Labadie_Summary_ADG_2024_method.csv
* ggin2024.csv
* FU2021_Number UWTV Stations.png
* FU2021_Abundance_estimates.png
* FU2021_meandensity.png
* FU2021_cvgeo.png
* FU2021_violin.plot.png
* FU2021_contour.plot.png

# Data Work Up
<a href="#top">Back to top</a>

This markdown document contains the full kriging procedure for FU 2021 for survey year `dat.year`.
...
Marine Institute UWTV survey reports are available @ http://hdl.handle.net/10793/1657
...
# Copy other data from bootstrap/data/UWTV to model/UWTV/
Files for Kriging and ADG summary file
```{r copy_data , echo=TRUE, warning=FALSE, fig.height=8, fig.width=8}
cp("bootstrap/data/UWTV/pol.Labadie.IBP.csv", "model/UWTV/pol.Labadie.IBP.csv")
cp("bootstrap/data/UWTV/europa.txt", "model/UWTV/europa.txt")

cp("bootstrap/data/UWTV/nep2021_2013_2024_densities.csv", "model/UWTV/nep2021_2013_2024_densities.csv") 
cp("bootstrap/data/UWTV/ggins_compiled_Labadie_contourplot.csv", "model/UWTV/ggins_compiled_Labadie_contourplot.csv")

cp("bootstrap/data/UWTV/Labadie_Summary_ADG_2023_method.csv", "model/UWTV/Labadie_Summary_ADG_2023_method.csv") 

cp("bootstrap/data/UWTV/UWTV_2024_Stations_updated.csv", "model/UWTV/UWTV_2024_Stations_updated.csv") 
...
## Load the UWTV survey abundance data.
<a href="#top">Back to top</a>
The data is available only in MI SQL database and it is extracted as a data object.
Summary of data. 

```{r summ, echo=TRUE, message=FALSE}

nep.all <- read.csv("model/UWTV/nep2021_2013_2024_densities.csv")
...
if (save.plots == T) {
ggsave("model/model_02_kriging/FU2021_violin.plot.density.png")
}
```
...
if (save.plots == T) {
ggsave("model/model_02_kriging/FU2021_violin.plot.counts.png")
}
```
...
if (save.plots == T) {
ggsave("model/model_02_kriging/FU2021_violin.plot.dog.png")
}
...
projec.toggle(mode=0)
  ...
  #png("model/model_02_kriging/FUXX_krig.datapoints.png", height=2000, width=1500, res=200)
...
```{r poly, echo=T, message=FALSE}
  #png("model/model_02_kriging/FUXX_krig.surveydomain.png", height=2000, width=1500, res=200)

pol.FU2021 <- read.table("model/UWTV/pol.Labadie.IBP.csv", header=T, sep=",")

poly <- polygon.create(x=pol.FU2021$x,y=pol.FU2021$y)
plot(poly)
plot(data.db,main=mt, add=T)
db.poly<-polygon.create(x=pol.FU2021[,1],y=pol.FU2021[,2],polygon=NA)
europa <- read.table("model/UWTV/europa.txt",header=T)
plot(data.db,pch=21, title = mt,
     inches=5,asp=1/cos(mean(db.extract(data.db,"x1"))*pi/180),
     xlim=c(-9.5,-6.65),ylim=c(49.5,51.2))
...
projec.toggle(mode=1)
  ...
  #png("model/model_02_kriging/FUXX_krig.polygon.check.png", height=2000, width=1500, res=200)
  plot(data.db,pch=21,title= mt,inches=5,asp=1,xlim=c(-50,50),ylim=c(-50,50))
plot(poly,add=T)
...
cat("mean: ",zm,"    var: ",zv,"   cv: ",sqrt(zv)/zm,"\n")
  ...
  #png("model/model_02_kriging/FUXX_krig.hist.densities.png", height=2000, width=1500, res=200)
...
vario.plot(vg1,npairpt=1,xlab="Distance",ylab="Variogram",pch=9,cex=0.001,col="grey", title=mt)
    ...
    #png("model/model_02_kriging/FUXX_krig.vg.fit.png", height=2000, width=1500, res=200)
  vg.fit=model.auto(vg1,struc=c("Spherical"),title = paste(mt, "auto fit Spherical"), xlab="Distance", ylab="variogram")
    #dev.off()
...
A grid of 100X100 points was choosed because it was similiar to the previous methodology used in SURFER.
...
poldat<-read.table("model/UWTV/pol.Labadie.IBP.csv",header=T,sep=',')
...
gd.disc=db.polygon(gd.disc,db.poly)
  ...
  #png("model/model_02_kriging/FU2021_krig.grid.data.png", height=2000, width=1500, res=200)
  plot(gd.disc,pch=3,col=1, xlim=c(-60,60),ylim=c(-60,60),title=mt);plot(db.c1,add=T,pch=21); plot(db.poly,add=T)
  #dev.off()
```
...
```{r krige_res1, echo=TRUE, message=TRUE, eval=TRUE, fig.cap="Kriged Density Adjusted.estimated."}
  #png("model/model_02_kriging/FU2021_krig.estimation.png", height=2000, width=1500, res=200)

plot(kri,col=tim.colors(200),asp=1,
     xlim=c(-60,60),ylim=c(-60,60),
name.image="Kriging.AdjustedBurrowDensity.estim")
plot(poly,add=T)
...
```{r krige_res2, echo=TRUE, message=TRUE, eval=TRUE, fig.cap="Kriged Density Adjusted.standard deviation."}  
  #png("model/model_02_kriging/FU2021_krig.stdev.png", height=2000, width=1500, res=200)
...
k.sum <- read.csv("model/UWTV/Labadie_Summary_ADG_2023_method.csv")



k.sum <-rbind(k.sum, data.frame(Year=mean(nep$Year), Ground ="Labadie", mean= zm,  N= db.c1$nech,  
...
k.sum$Ground <- "Labadie"
k.sum$FU <- fu.n

fileOut <- paste("model/model_02_kriging/Labadie_Summary_ADG_method_", curr.year, ".csv", sep = "")

if (save.tables == T) {
    write.table(k.sum, fileOut, sep=",", row.names = F)
}
...
data.db <- xvalid(db.c1,model=vg.fit,uniquenei)
  ...
  #png("model/model_02_kriging/FU2021_krig_crossval.png", height=2000, width=1500, res=200)
  
hist(db.extract(data.db,"Xvalid.AdjustedBurrowDensity.esterr"),nclass=30,main="nep.fu.2021.CrossValidation",xlab="Cross validation error",col="blue")
...
sum <- k.sum %>% filter(Year >= 2013)

  ggplot(sum, aes(x=Year,y=N)) + geom_line(size=1) +  geom_point() +        
    theme_bw(base_size=12) +
          scale_x_continuous(name="\nYear",
                             breaks = seq(min(sum$Year), max(sum$Year), 1)) +
          scale_y_continuous(name = "Number of Stations \n",
                             breaks = seq(0, max(sum$N)+10, 10),
                             limits = c(0, max(sum$N)+10))+
          theme(panel.grid=element_blank(), legend.position = "bottom")  +
          theme(legend.title=element_blank())  
  
if (save.plots == T) {
  ggsave("model/model_02_kriging/FU2021_Number UWTV Stations.png")
}
```
...
ggplot(sum, aes(x=Year, y= abund)) +
          ...
          scale_y_continuous(name = "Abundance (millions)\n",
                             breaks = seq(0, max(sum$upper)+100, 250),
                             limits = c(0, max(sum$upper)+100)) + 
          geom_hline(aes(yintercept=450),colour="#990000",linetype="dashed",size = 0.9) 


if (save.plots == T) {
  ggsave("model/model_02_kriging/FU2021_Abundance_estimates.png")
}
```
...
```{r final_res3, echo=TRUE, message=TRUE, eval=TRUE, fig.cap="Uncertainty estimate. Dashed line is 20% limit."}
  ...
  #png("model/model_02_kriging/FUXX_UWTV Uncertainty Estimate.png", height=2000, width=1500, res=200)  
ggplot(sum, aes(x=Year, y= CVgeo)) +
          theme_bw(base_size=12) +
          geom_line(size = 1) +
          geom_point() +
          theme_bw() +
          ...
          scale_y_continuous(name = "CV Geo",
                             breaks = seq(0, max(sum$CVgeo)+0.25, 0.01),
                             limits = c(0, max(sum$CVgeo)+0.25))  +
          theme(panel.grid=element_blank(), legend.position = "bottom")  +
          theme(legend.title=element_blank())  
  #dev.off()

if (save.plots == T) {
  ggsave("model/model_02_kriging/FU2021_uncertainty_estimate.png")
}
```

### Mean Density estimate (burrow/m-2)

Due to incomplete survey coverage the 2013 mean density estimate is not presented.
As the total abundance is scaled up from the area covered by the survey in that year.

```{r final_res4, echo=TRUE, message=TRUE, eval=TRUE, fig.cap="Mean density estimate (burrow/m2)."}
  #png("model/model_02_kriging/FUXX_UWTV Mean Density.png", height=2000, width=1500, res=200)

sum <- sum %>% filter(Year > '2013')
...
ggplot(sum, aes(x=Year, y= mean)) +
          ...
          scale_y_continuous(name = "Mean Density (burrow/m-2)",
                             breaks = seq(0, max(sum$mean)+0.3, 0.1),
...
if (save.plots == T) {
  ggsave("model/model_02_kriging/FU2021_meandensity.png")
}
```
...
```{r contour, echo=TRUE, message=TRUE, fig.height= 10, fig.width=7, eval=TRUE, fig.cap="Kriged contour and density bubble plot."}
ggin <- read.csv("model/UWTV/ggins_compiled_Labadie_contourplot.csv")


shapefile_path_FG <- "C:/GitHub/2024_nep.fu.2021_assessment/model/shp/nep.2021.ground.shp"
...
p <- ggplot(data = FG) +
   ...
   facet_wrap(~year, nrow = 4)  # Facet by year


names(nep.all)[2] <- "year"
names(nep.all)[12] <- "mid_lon"
names(nep.all)[13] <- "mid_lat" 
names(nep.all)[21] <- "Density"

b <- geom_point(data=nep.all, aes(x=mid_lon, y=mid_lat, size=Density), shape =1)
...
if (save.plots == T) {
  ggsave("model/model_02_kriging/FU2021_contour.plot.png")
}
```
